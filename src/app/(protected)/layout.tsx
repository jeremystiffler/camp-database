...
function ProtectedLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="protected-shell min-h-dvh w-full flex items-stretch text-slate-900" style={workspaceStyle}>
      ...
      {/* Sidebar */}
      {!isKioskShell && <aside
        className={`fixed top-0 left-0 h-full w-64 border-r border-blue-100 bg-blue-50/90 text-slate-900 shadow-xl shadow-blue-100/60 backdrop-blur flex flex-col z-30 transition-transform duration-200 ${
      ...
      >
        ...
        {camps.length > 0 && (
          <div className="px-3 py-4 border-b border-slate-100">
            ...
            <div className="rounded-2xl border border-blue-200 bg-white p-3 shadow-sm shadow-blue-100/70">
                ...
                {camps.map((camp) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left rounded-xl px-3 py-2.5 transition-colors ${selected ? "bg-blue-100 text-blue-950" : "text-slate-700 hover:bg-blue-50"}`}
                    >
        ...
        <nav aria-label="Event navigation" className="flex-1 px-3 py-4 space-y-3 overflow-y-auto">
          <div>
            ...
            <div className="space-y-1">{primaryNav.filter((item) => roleRank(activeCamp?.myRole) >= roleRank(item.minRole)).map((item) => {
              ...
              return <Link key={item.href} href={navHref(item.href)} aria-current={isActive ? "page" : undefined} onClick={() => setSidebarOpen(false)} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all duration-150 ${isActive ? "bg-blue-100 text-blue-950 shadow-sm" : "text-slate-600 hover:bg-blue-50 hover:text-blue-900"}`}><span className={`w-6 h-6 rounded-lg flex items-center justify-center ${isActive ? "bg-blue-600 text-white" : "bg-blue-100 text-blue-700"}`}><SidebarIcon name={item.icon} /></span>{item.label}</Link>;
            })}</div>
          </div>
          <div>
          ...
          {user.isSuperAdmin && <Link href="/super-admin" onClick={() => setSidebarOpen(false)} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all duration-150 ${pathname.startsWith("/super-admin") ? "bg-blue-100 text-blue-950 shadow-sm" : "text-slate-600 hover:bg-blue-50 hover:text-blue-900"}`}><span className={`w-6 h-6 rounded-lg flex items-center justify-center ${pathname.startsWith("/super-admin") ? "bg-blue-600 text-white" : "bg-blue-100 text-blue-700"}`}><SidebarIcon name="gear" /></span>Super admin</Link>}
        </nav>

        {/* User footer */}
        <div className="px-3 py-4 border-t border-slate-100">
          <div className="flex items-center gap-3 px-2">
            <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 shadow-sm shadow-blue-200">
      ...
      <main className={`flex-1 min-w-0 min-h-dvh flex justify-center ${isKioskShell ? "pt-0" : "lg:ml-64 pt-14 lg:pt-0"}`} style={{ background: "var(--canvas-sunk)" }}>
