...
function DashboardContent() {
  ...
  if (guidedMode) {
    ...
    const progress = [
      ...
      [Boolean(summary?.stats?.classes), "Things kids do"],
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {activeCamp && <div className="relative">
              ...
              {actionsOpen && <div className="absolute right-0 top-10 z-30 w-[min(92vw,340px)] rounded-2xl border border-slate-200 bg-white p-3 shadow-2xl">
                ...
                {canEditCamp(activeCamp) && <div className="mt-2 border-t border-slate-200 pt-2"><button type="button" onClick={() => { setActionsOpen(false); setCopyingCamp(activeCamp); }} className="w-full rounded-xl px-3 py-2 text-left text-sm font-black !text-slate-800 hover:bg-slate-50">Duplicate</button></div>}
              </div>}
            </div>}
