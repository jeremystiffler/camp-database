...
function DashboardContent() {
  ...
  if (guidedMode) {
    ...
    return <div className="mx-auto max-w-3xl space-y-6">
      ...
      <section className="camp-card p-6"><h2 className="text-lg font-black text-slate-900">Your progress</h2><div className="mt-4 space-y-3">{progress.map(([done, label]) => <div key={String(label)} className="flex items-center gap-3 text-sm font-bold text-slate-700"><span className={done ? "text-emerald-600" : "text-slate-400"}>{done ? "✓" : "○"}</span>{label}{!done && label === "Things kids do" && <span className="text-xs font-semibold text-indigo-600">up next</span>}</div>)}</div></section>
