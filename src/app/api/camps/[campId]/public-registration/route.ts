...
async function sendConfirmationEmail({
  ...
  const paymentHtml = paymentRequired
    ? `<h2 style="font-size:16px;margin:18px 0 8px;">Payment Summary</h2><p style="margin:0;">Event price: ${money(totals.campPriceCents)}<br>Discount: ${money(totals.discountCents)}<br>Platform fee: ${money(totals.platformFeeCents)}<br><strong>Total due: ${money(totals.totalCents)}</strong></p>`
    : "";
  ...
  const renderBlock = (block: EmailTemplateBlock) => {
    ...
    if (block.type === "footer") return `<p style="margin-top:22px;color:#64748b;font-size:13px;">${textToHtml(content || "Need to change something? Contact the event office.")}</p>`;
    return "";
  };
...
async function sendAdminNotificationEmail({
  ...
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;line-height:1.5;color:#1e293b;max-width:680px;margin:0 auto;padding:24px;">
      ...
      <p style="margin:0;">${paymentRequired ? `Payment required: <strong>${money(totals.totalCents)}</strong>` : "No payment required at submission."}<br>Event price: ${money(totals.campPriceCents)}<br>Discount: ${money(totals.discountCents)}<br>Platform fee: ${money(totals.platformFeeCents)}</p>
    </div>`;
