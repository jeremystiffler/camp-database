...
export default function ParticipantScannableCode({ value, label = "Participant QR", size = 148, className = "" }: Props) {
  ...
  if (!value) {
    return <div className={`rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-4 text-center text-xs font-bold text-slate-500 ${className}`}>No scan code yet</div>;
  }
