...
export function HelpModeToggle({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-xl border text-xs font-extrabold transition ${helpMode ? "border-sky-200 bg-sky-50 text-sky-800" : "border-slate-200 bg-white text-slate-500 hover:bg-slate-50"} ${compact ? "px-2.5 py-1.5" : "px-3 py-2"}`}
  ...
  return (
    <span className="relative inline-flex align-middle">
      <button
        ...
        className="ml-1 inline-flex h-5 w-5 items-center justify-center rounded-full border border-slate-200 bg-white text-[11px] font-extrabold text-slate-500 shadow-sm hover:border-sky-200 hover:text-sky-700"
        aria-label={title}
      >
        ?
      </button>
