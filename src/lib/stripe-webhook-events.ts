...
async function completeCheckout(session: Stripe.Checkout.Session, stripeAccountId: string | null) {
  ...
  if (session.metadata?.type !== "participant_registration" || !paymentId || !stripeAccountId || session.payment_status !== "paid") return;
...
async function failCheckout(session: Stripe.Checkout.Session, stripeAccountId: string | null, code: string) {
  const paymentId = paymentIdFromMetadata(session.metadata);
  if (session.metadata?.type !== "participant_registration" || !paymentId || !stripeAccountId) return;
  await failPendingRegistrationPayment({ paymentId, stripeAccountId, code });
}

async function updatePaymentAndParticipantsByIntent(paymentIntentId: string, stripeAccountId: string, status: string, fullyRefunded = false) {
  ...
  await prisma.participant.updateMany({
    where: { id: { in: payments.flatMap(payment => payment.participants.map(link => link.participantId)) } },
  ...
  try {
    switch (event.type) {
      ...
      case "charge.refunded": {
        ...
        if (paymentIntentId && stripeAccountId) await updatePaymentAndParticipantsByIntent(paymentIntentId, stripeAccountId, charge.refunded ? "refunded" : "partially_refunded", charge.refunded);
        break;
      }
      case "charge.dispute.created": {
        const dispute = event.data.object as Stripe.Dispute;
        const paymentIntentId = typeof dispute.payment_intent === "string" ? dispute.payment_intent : dispute.payment_intent?.id;
        if (paymentIntentId && stripeAccountId) await updatePaymentAndParticipantsByIntent(paymentIntentId, stripeAccountId, "disputed");
        break;
      }
    }
  } catch (error) {
